    ...
    post("/submit", (request,response) -> {
      ...
      try {
        ...
        return sw.toString();
    	  ...
    	  logger.error("The job threw an exception, which was:", mue.getCause());
          StringWriter sw = new StringWriter();
          mue.getCause().printStackTrace(new PrintWriter(sw));
          response.status(500, "Job threw an exception");
          return sw.toString();
      } catch (ClassNotFoundException cne) {
    	  response.status(400, "Bad request");
          return "Double-check that the class "+className+" exists";
      }
